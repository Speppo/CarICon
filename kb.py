...
def load_data_in_kb():
    ...
    # Discontinuità sintattica delle righe nei file Prolog
...
# Inserisco una lista di fatti all'interno di un file
...
def create_kb() -> Prolog:
    ...
    # Ipotizzo e fisso una velocità di 100kmh e una forza media di resistenza aerodinamica di 450N
    ...
    # Calcolo capacità media del bagagliaio delle auto di un certo brand
    ...
    # Modello più potente di un certo brand
    ...
    # Elemento più frequente all'interno di una lista
    prolog.assertz("most_frequent_element(List, Element) :-"
                   
                   # findall: crea una lista di coppie ed ogni coppia è avvalorata dal conteggio degli elementi della 
                   ...
                   # keysort: ordina la lista di coppie in modo crescente (Pairs)
                   "  keysort(Pairs, SortedPairs),"
                   
                   # reverse: capovolge la lista per poi estrae con [Count-Element|_] e preleva solo il primo elemento della lista 
                   # (la testa Count-Element) ed ignora la coda (_) 
